...
Nutrient pressure is calculated from area-weighted, right-continuous baseline ECDFs of NO3 and PO4, with `cos(latitude)` weights. These nutrient reference distributions are frozen from the complete valid baseline projection table before the four-scenario coordinate intersection. Future nutrient values are evaluated against those baseline references and combined using the same frozen baseline nutrient weights.
...
`scripts/09_trait_constraint_analysis.py` evaluates trait selection, Rao's Q and functional substitution. All 30 biological species remain in model-derived opportunity and winner calculations, while functional inference uses the direct-trait complete-case species set. Primary functional inference is restricted to regional candidate pools with at least 80% trait-resolved coverage. The trait-substitution gap is the minimum functional distance from the unconstrained best-performing species to a trait-resolved species in the regional pool; zero denotes an exact functional match. Adjusted alignment models test whether overall Rao's Q adds information on mean trait-substitution gap or exact-match share after accounting for richness, environmental context, scenario and realm.

## 10. EEZ aggregation

`scripts/10_aggregate_eez.py`

Each 0.25° model cell is intersected only with ordinary Marine Regions EEZ features (`POL_TYPE == "200NM"`) and weighted by `cos(latitude) × geodesic overlap fraction`. Global baseline Q75 thresholds are retained. Overlapping retained EEZs are aggregated independently and are not renormalized between jurisdictions. Cross-scenario outputs include persistent mismatch, future-specific emerging and recovered mismatch, baseline biophysical opportunity and future-minus-baseline mismatch changes.

Formal inference requires at least 100 valid pixels and five independent 5° blocks. Spatial-block sign-flip permutations and block bootstrap intervals are combined with scenario-specific Benjamini–Hochberg correction. A formally supported directional change must exceed 1 percentage point, have a same-direction 95% bootstrap interval excluding zero, and have FDR-adjusted `P < 0.05`. Sovereign GDP per capita is joined only when an EEZ resolves unambiguously to one sovereign ISO3 code and is retained as broad economic context rather than a causal predictor.
