...
def weighted_sd(x, w) -> float:
    ...
    if not np.isfinite(mean):
        return np.nan
    return float(np.sqrt(weighted_mean((np.asarray(x, float) - mean) ** 2, w)))


def standardize(x, w) -> np.ndarray:
    mean, sd = weighted_mean(x, w), weighted_sd(x, w)
    if np.isfinite(sd) and sd > 0:
        return (np.asarray(x, float) - mean) / sd
...
def subset_r2(x: np.ndarray, y: np.ndarray, w: np.ndarray, columns: tuple[int, ...]) -> float:
    if not columns:
        return 0.0
    return weighted_fit(x[:, columns], y, w)[0]
...
def shapley_r2(x: np.ndarray, y: np.ndarray, w: np.ndarray) -> tuple[float, np.ndarray]:
    ...
    for size in range(1, p + 1):
        for subset in combinations(range(p), size):
            cache[subset] = subset_r2(x, y, w, subset)
...
def partial_permutation_p(x, y, controls, w, block, observed, rng) -> float:
    ...
    for _ in range(N_PERM):
        ...
        for sign, rows in zip(signs, groups):
            signed[rows] *= sign
...
def bootstrap_partial(x, y, controls, w, block, rng) -> tuple[float, float]:
    ...
    for i in range(N_BOOT):
        ...
        rows = np.concatenate([groups[j] for j in draw])
...
def prepare_data() -> pd.DataFrame:
    ...
    out = drivers.merge(
        base[["lon_key", "lat_key", "area_weight", *RESPONSES]],
        on=["lon_key", "lat_key"], how="inner", validate="one_to_one",
        suffixes=("", "_baseline_response"),
    )
    ...
    for scenario in FUTURES:
        ...
        keep = future[["lon_key", "lat_key", *RESPONSES]].rename(
            columns={response: f"{response}_{scenario}" for response in RESPONSES}
        )
        out = out.merge(keep, on=["lon_key", "lat_key"], how="inner", validate="one_to_one")
        for response in RESPONSES:
            out[f"delta_{response}_{scenario}"] = (
                out[f"{response}_{scenario}"] - out[f"{response}_baseline"]
            )
            ...
            rows.append(
                {
                    "scenario": scenario,
                    "driver": driver,
                    "mean_change": weighted_mean(delta, w),
                    "baseline_spatial_sd": sd,
                    "standardized_mean_change": weighted_mean(delta, w) / sd if sd > 0 else np.nan,
                }
            )
        ...
        for response in RESPONSES:
            ...
            for driver, value in zip(DRIVERS, contributions):
                attribution_rows.append(
                    {
                        "scenario": scenario, "response": response, "driver": driver,
                        "model_r2": full_r2, "shapley_r2": value,
