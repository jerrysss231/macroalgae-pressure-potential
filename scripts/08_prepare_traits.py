...
STATE_PATTERNS = {
    67: [
        ("epilithic", r"\bepilithic\b"),
        ("epiphytic", r"\bepiphytic\b"),
        ("epizoic", r"\bepizoic\b"),
        ("unattached", r"\bunattached\b"),
    ],
    72: [
        ("annual", r"\bannual\b"),
        ("ephemeral", r"\bephemeral\b"),
        ("perennial", r"\bperennial\b"),
    ],
    73: [
        (state, rf"\b{state}\b")
        for state in ("erect", "branched", "tubular", "foliose", "filamentous")
    ],
    76: [
        ("siphonous", r"\bsiphonous\b"),
        ("non_unicellular", r"non-unicellular"),
    ],
    78: [
        ("dioecious", r"\bdioicous\b|\bdioecious\b"),
        ("monoecious", r"\bmonoicous\b|\bmonoecious\b"),
        ("mixed", r"\bmixed\b"),
    ],
    79: [("yes", r"\byes\b"), ("no", r"\bno\b")],
    82: [
        ("oogamous", r"\boogamous\b"),
        ("anisogamous", r"\banisogamous\b"),
        ("isogamous", r"\bisogamous\b"),
    ],
    84: [
        ("haplodiplontic", r"\bhaplodiplontic\b"),
        ("diplontic", r"\bdiplontic\b"),
        ("isomorphic", r"\bisomorphic\b"),
        ("heteromorphic", r"\bheteromorphic\b"),
    ],
    85: [
        ("water_column", r"fertilisation in the water column"),
        ("female_gametophyte", r"fertilisation on female gametophyte"),
    ],
}
...
def clean_traits(raw: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for attribute, (trait, dimension) in TRAITS.items():
        clean[trait] = raw[columns[attribute]].map(
            lambda value, aid=attribute: canonical_states(value, aid)
        )
        metadata.append(
            {
                "attribute_id": attribute,
                "raw_column": columns[attribute],
                "clean_trait": trait,
                "functional_dimension": dimension,
                "trait_type": "multistate",
                "coverage": float(clean[trait].notna().mean()),
            }
        )
    ...
    for i in range(len(clean)):
        ...
        for j in range(i + 1, len(clean)):
            ...
            for dimension, traits in dimensions.items():
                values = [
                    jaccard_dissimilarity(clean.at[i, trait], clean.at[j, trait])
                    for trait in traits
                ]
                values = [value for value in values if np.isfinite(value)]
                if values:
                    value = float(np.mean(values))
                    dimension_matrices[dimension][i, j] = value
                    dimension_matrices[dimension][j, i] = value
...
def main() -> None:
    ...
    pd.DataFrame(shared, index=names, columns=names).to_csv(
        OUT_DIR / "shared_functional_dimensions.csv"
    )
    for dimension, matrix in dimension_matrices.items():
        pd.DataFrame(matrix, index=names, columns=names).to_csv(
            OUT_DIR / f"functional_distance_{dimension}.csv"
        )

    complete = clean[list(name for name, _ in TRAITS.values())].notna().all(axis=1)
    summary = {
        "n_species": int(len(clean)),
        "n_complete_primary_trait_species": int(complete.sum()),
        "primary_traits": list(name for name, _ in TRAITS.values()),
        "functional_dimensions": sorted(metadata["functional_dimension"].unique().tolist()),
        "minimum_shared_dimensions": MIN_SHARED_DIMENSIONS,
        "distance_rule": (
            "mean trait dissimilarity within dimension, then mean across shared dimensions"
        ),
        "missing_trait_imputation": False,
    }
    (OUT_DIR / "trait_preparation_summary.json").write_text(
